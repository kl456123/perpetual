...
export class WebsocketService {
  ...
  public start() {
    ...
    this.eventManager.on(
      ...
      (tradesHistory: TradeHistory) => {
        this.tradeRecordUpdate([tradesHistory]);
      }
    );
  }
  ...
  public tradeRecordUpdate(tradesHistory: TradeHistory[]) {
      ...
      for (const [requestId, orders] of Object.entries(requestIdToOrders)) {
        ...
        if (ws) {
          ws.send(
            JSON.stringify({
              ...response,
              payload: Array.from(orders).map(item => ({
                hash: item.metaData.orderHash,
                isBuy: item.order.isBuy,
                price: item.order.limitPrice.value.toString(),
                size: item.order.amount
                  .minus(item.metaData.filledAmount)
                  .toFixed(0),
              })),
              requestId,
            })
  ...
  public async makeInitialResponse(
    ...
    if (subscriptionOpts.channel === MessageChannels.TradeHistory) {
      ...
      paginatedTradesHistory.records.forEach(record =>
        this.eventManager.emitTradeRecord(record)
      );
    }
  }
  ...
  private _processMessage(ws: WrappedWebSocket, data: WebSocket.Data): void {
    ...
    switch (type) {
      case MessageTypes.Subscribe: {
        ...
        this.makeInitialResponse(requestId, subscriptionOpts);
        break;
      }
      default:
